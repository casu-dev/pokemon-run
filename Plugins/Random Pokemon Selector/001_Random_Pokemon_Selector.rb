...
# Unused
def pbComparePool
    original = getOakPool7
    input = original.dup
    nfe = getGen8NFE
    input -= nfe
     pbMessage("Evolved count "+input.length.to_s)
     pbWriteIntoFile("evolved.txt", "%i"+input.to_s.gsub(/[:\,]/, ''))
     pbRemoveFromPool(original, input)
end

def pbRemoveFromPool(pool, blackList)
    pool -= blackList
     pbMessage("Cut count "+pool.length.to_s)
     pbWriteIntoFile("poolCut.txt", "%i"+pool.to_s.gsub(/[:\,]/, ''))
end
...
def pbgetMonoTypePool(type)
    ...
    regionals = basePool.intersection(getRegionals)
    regionals.each do |r|
       output.push(r) if (type == (GameData::Species.get_species_form(r,1).type1) || type == (GameData::Species.get_species_form(r,1).type2))
    end

    if basePool.include? :MEOWTH
        r=:MEOWTH
        output.push(:MEOWTH) if (type == (GameData::Species.get_species_form(r,2).type1) || type == (GameData::Species.get_species_form(r,2).type2))
    end

    if basePool.include? :ROTOM
        r=:ROTOM
        (0...6).each do |i|
            output.push(:ROTOM) if (type == (GameData::Species.get_species_form(r,i).type1) || type == (GameData::Species.get_species_form(r,i).type2))
        end
    end

    if basePool.include? :ORICORIO
        r=:ORICORIO
        (0...4).each do |i|
            output.push(r) if (type == (GameData::Species.get_species_form(r,i).type1) || type == (GameData::Species.get_species_form(r,i).type2))
        end
    end
...
#Count:33
def getRegionals
%i[RATTATA RATICATE RAICHU SANDSHREW SANDSLASH VULPIX NINETALES DIGLETT DUGTRIO MEOWTH PERSIAN GEODUDE GRAVELER GOLEM PONYTA RAPIDASH SLOWPOKE SLOWBRO FARFETCHD GRIMER MUK EXEGGUTOR MAROWAK WEEZING MRMIME SLOWKING CORSOLA ZIGZAGOON LINOONE DARUMAKA DARMANITAN YAMASK STUNFISK]
end
...
%i[VENUSAUR CHARIZARD BLASTOISE BEEDRILL PIDGEOT ALAKAZAM SLOWBRO GENGAR KANGASKHAN PINSIR GYARADOS AERODACTYL
    ...
    SHARPEDO CAMERUPT ALTARIA BANETTE ABSOL GLALIE SALAMENCE METAGROSS LOPUNNY GARCHOMP LUCARIO ABOMASNOW GALLADE AUDINO HOUNDOOM]
end
##### Legi #####
#Count:85
...
def getOakPool1
%i[CATERPIE METAPOD WEEDLE KAKUNA MAGIKARP SENTRET PICHU CLEFFA SMEARGLE IGGLYBUFF WOOPER SUNKERN TYROGUE POOCHYENA ZIGZAGOON WURMPLE SILCOON CASCOON LOTAD SEEDOT RALTS WHISMUR MAKUHITA AZURILL FEEBAS KRICKETOT BURMY HAPPINY BUNNELBY SCATTERBUG SPEWPA BOUNSWEET WIMPOD BLIPBUG ROLYCOLY SNOM]
end
...
def getOakPoolLW1
%i[RATTATA SPEAROW JIGGLYPUFF DIGLETT HOOTHOOT LEDYBA SPINARAK MARILL SLUGMA TAILLOW WINGULL SURSKIT NINCADA SKITTY WYNAUT BIDOOF COMBEE CHERUBI PATRAT NOIBAT YUNGOOS DEWPIDER FOMANTIS SKWOVET NICKIT GOSSIFLEUR WOOLOO YAMPER APPLIN TOXEL MILCERY PICHU CLEFFA IGGLYBUFF ZIGZAGOON WURMPLE LOTAD SEEDOT RALTS WHISMUR AZURILL HAPPINY SCATTERBUG BOUNSWEET BLIPBUG ROLYCOLY]
end

# 242 <= BST <= 277, Odds F1: Rare
def getOakPoolLW2
%i[EKANS VULPIX PARAS MEOWTH PINECO KIRLIA SHROOMISH MEDITITE ELECTRIKE BARBOACH SHUPPET CHINGLING BONSLY RIOLU PURRLOIN MUNNA BLITZLE COTTONEE PETILIL FOONGUS INKAY HELIOPTILE ROCKRUFF MORELULL STEENEE CHEWTLE ARROKUDA PIDGEY NIDORANfE NIDORANmA ZUBAT TOGEPI HOPPIP SWINUB STARLY SHINX LILLIPUP PIDOVE VENIPEDE TYNAMO LITWICK PIKIPEK ROOKIDEE HATENNA IMPIDIMP DREEPY]
end

# 278 <= BST <= 299, Odds F1: Often
def getOakPoolLW3
%i[PIDGEOTTO PIKACHU SANDSHREW NIDORINA NIDORINO CLEFAIRY VENONAT PSYDUCK MANKEY GROWLITHE TENTACOOL SLOWPOKE DODUO SEEL GRIMER SHELLDER DROWZEE KRABBY VOLTORB EXEGGCUTE CUBONE KOFFING GOLDEEN STARYU EEVEE OMANYTE KABUTO FLAAFFY SKIPLOOM AIPOM SNUBBULL SMOOCHUM NATU REMORAID CHINCHOU TEDDIURSA PHANPY HOUNDOUR LOMBRE NUZLEAF LOUDRED NOSEPASS GULPIN CARVANHA NUMEL SPOINK VIBRAVA CACNEA SWABLU CORPHISH BALTOY LILEEP ANORITH SNORUNT CLAMPERL STARAVIA LUXIO CRANIDOS SHIELDON BUIZEL SHELLOS DRIFLOON BUNEARY GLAMEOW STUNKY BRONZOR HIPPOPOTAS SKORUPI CROAGUNK FINNEON MANTYKE SNOVER HERDIER PANSAGE PANSEAR PANPOUR TRANQUILL WOOBAT DRILBUR WHIRLIPEDE KROKOROK DARUMAKA DWEBBLE SCRAGGY YAMASK TIRTOUGA TRUBBISH ZORUA MINCCINO DUOSION DUCKLETT DEERLING KARRABLAST FRILLISH JOLTIK FERROSEED ELGYEM LAMPENT CUBCHOO SHELMET MIENFOO GOLETT PAWNIARD RUFFLET VULLABY LARVESTA LITLEO FLOETTE SKIDDO PANCHAM ESPURR SPRITZEE SWIRLIX BINACLE SKRELP CLAUNCHER TYRUNT AMAURA PHANTUMP PUMPKABOO BERGMITE TRUMBEAK CRABRAWLER CUTIEFLY MAREANIE SALANDIT STUFFUL SANDYGAST CORVISQUIRE DOTTLER SILICOBRA SIZZLIPEDE CLOBBOPUS SINISTEA HATTREM MORGREM CUFANT HORSEA MAREEP SLAKOTH TRAPINCH DUSKULL SPHEAL BUDEW ROGGENROLA TYMPOLE SANDILE GOTHITA SOLOSIS FLETCHLING]
end

# 300 <= BST <= 376, Odds F1: Rare, Odds F2: Rare
def getOakPoolLW4
%i[BULBASAUR CHARMANDER SQUIRTLE ODDISH POLIWAG ABRA MACHOP BELLSPROUT GEODUDE MAGNEMITE GASTLY RHYHORN DRATINI CHIKORITA CYNDAQUIL TOTODILE MAGBY ELEKID LARVITAR TREECKO TORCHIC MUDKIP ARON BAGON BELDUM TURTWIG CHIMCHAR PIPLUP MIMEJR GIBLE SNIVY TEPIG OSHAWOTT TIMBURR SEWADDLE VANILLITE KLINK AXEW DEINO CHESPIN FENNEKIN FROAKIE FLABEBE HONEDGE GOOMY ROWLET LITTEN POPPLIO GRUBBIN JANGMOO GROOKEY SCORBUNNY SOBBLE]
end

# 377 <= BST <= 399, Odds F2: Often, Odds F3: Rare
def getOakPoolLW5
%i[GLOOM POLIWHIRL WEEPINBELL GRAVELER FARFETCHD ONIX LICKITUNG YANMA MUNCHLAX BOLDORE PALPITOAD SWADLOON GOTHORITA VANILLISH FLETCHINDER MUDBRAY]
end

# 400 <= BST <= 409, Odds F2: Rare, Odds F3: Often, Odds F4: Rare
def getOakPoolLW6
%i[PORYGON IVYSAUR CHARMELEON WARTORTLE KADABRA MACHOKE HAUNTER BAYLEEF QUILAVA CROCONAW TOGETIC MURKROW GROVYLE COMBUSKEN MARSHTOMP ROSELIA WAILMER GROTLE MONFERNO PRINPLUP GURDURR ARCHEN EELEKTRIK QUILLADIN BRAIXEN FROGADIER CHARJABUG]
end
...
def getOak5Merged6
%i[SHEDINJA DITTO RATICATE PARASECT FURRET ARIADOS DUNSPARCE SUDOWOODO WOBBUFFET DELCATTY MEDICHAM PLUSLE MINUN BIBAREL PACHIRISU CHATOT VIVILLON GUMSHOOS SHIINOTIC PYUKUMUKU GLOOM POLIWHIRL WEEPINBELL GRAVELER FARFETCHD ONIX LICKITUNG PORYGON YANMA MUNCHLAX BOLDORE PALPITOAD SWADLOON GOTHORITA VANILLISH FLETCHINDER MUDBRAY FEAROW ARBOK SANDSLASH WIGGLYTUFF VENOMOTH DUGTRIO PERSIAN PRIMEAPE MAROWAK HITMONLEE HITMONCHAN SEAKING JYNX NOCTOWL QUAGSIRE JUMPLUFF SUNFLORA GRANBULL AZUMARILL HITMONTOP GIRAFARIG QWILFISH LANTURN MAGCARGO MIGHTYENA SWELLOW PELIPPER MASQUERAIN BRELOOM NINJASK SHARPEDO CAMERUPT ZANGOOSE SEVIPER LUNATONE SOLROCK CASTFORM KECLEON BANETTE TROPIUS CHIMECHO WORMADAM MOTHIM CHERRIM PURUGLY CARNIVINE LUMINEON ROTOM WATCHOG LIEPARD SWOOBAT AUDINO BASCULIN MARACTUS EMOLGA DIGGERSBY AROMATISSE DEDENNE ARAQUANID MINIOR TOGEDEMARU GREEDENT THIEVUL ELDEGOSS PERRSERKER PINCURCHIN MORPEKO IVYSAUR CHARMELEON WARTORTLE KADABRA MACHOKE PONYTA HAUNTER BAYLEEF QUILAVA CROCONAW TOGETIC CORSOLA MURKROW PUPITAR GROVYLE COMBUSKEN MARSHTOMP ROSELIA WAILMER SEALEO GROTLE MONFERNO PRINPLUP GABITE SERVINE PIGNITE DEWOTT GURDURR ARCHEN EELEKTRIK FRAXURE QUILLADIN BRAIXEN FROGADIER CHARJABUG CARKOL DRAKLOAK]
end

def getOak8Merged9
%i[RAICHU CLEFABLE HYPNO AMBIPOM OCTILLERY MANTINE LUDICOLO SHIFTRY GLALIE HUNTAIL GOREBYSS RELICANTH STARAPTOR LOPUNNY SPIRITOMB FROSLASS MUSHARNA UNFEZANT SCOLIPEDE WHIMSICOTT LILLIGANT DARMANITAN CRUSTLE SCRAFTY COFAGRIGUS JELLICENT BEHEEYEM DRUDDIGON GOLURK HEATMOR DURANT SLURPUFF MALAMAR HELIOLISK TOUCANNON LYCANROC LURANTIS SALAZZLE COMFEY PALOSSAND KOMALA TURTONATOR DRAMPA DREDNAW FLAPPLE APPLETUN GRAPPLOCT RUNERIGUS PIDGEOT DODRIO DEWGONG KINGLER FORRETRESS STANTLER XATU SKARMORY HARIYAMA MANECTRIC SWALOT TORKOAL GRUMPIG CACTURNE WHISCASH CRAWDAUNT ABSOL VESPIQUEN GASTRODON SKUNTANK THROH SAWK GARBODOR CINCCINO SWANNA SAWSBUCK AMOONGUSS ALOMOMOLA GALVANTULA STUNFISK FURFROU MEOWSTIC KLEFKI TREVENANT CRABOMINABLE ORICORIO RIBOMBEE MIMIKYU BRUXISH CRAMORANT FALINKS FROSMOTH STONJOURNER EISCUE INDEEDEE VILEPLUME VICTREEBEL GOLEM SLOWBRO ELECTRODE WEEZING KANGASKHAN TAUROS OMASTAR KABUTOPS SLOWKING BELLOSSOM MILTANK EXPLOUD ALTARIA CRADILY ARMALDO RAMPARDOS BASTIODON FLOATZEL DRIFBLIM MISMAGIUS TOXICROAK ABOMASNOW SIMISAGE SIMISEAR SIMIPOUR ZEBSTRIKA SIGILYPH CARRACOSTA GOTHITELLE REUNICLUS ESCAVALIER FERROTHORN ACCELGOR BISHARP BOUFFALANT TALONFLAME PANGORO DRAGALGE GOURGEIST TOXAPEX ORANGURU PASSIMIAN CORVIKNIGHT DUBWOOL BOLTUND BARRASKEWDA ALCREMIE]
end